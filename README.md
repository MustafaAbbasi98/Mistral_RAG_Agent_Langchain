...
- I also had to use custom prompt engineering to improve Mistral's tool-calling ability. Through experiments, I found out that a `react-json` agent that outputs JSON (name of tool + arguments) works better than the typical free-form text, `react`-only agent.
...
### Limitations
- The app does not include memory for a full chat experience. I tried to integrate memory into the agent but the LLM ignores the chat history always tries to a invole a tool even if it is not needed.
- The agent struggles with multi-step and multi-tool tasks. This is likely due to the weakness of the Mistral model's function-calling capabilities compared to stronger OpenAI models.
